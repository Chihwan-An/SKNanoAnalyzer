...
def get_hist_from_file(file_path, sys_dir, hist_name, x_max, custom_bins=None, rebin_factor=1):
    ...
    # 만약 x_max가 히스토그램 범위보다 크면 히스토그램의 실제 최댓값 사용
...
def run_plot():
    ...
    if h_total_mc := next((h for h in group_hists.values() if h), None):
    ...
    print(f">> Range: X=[{X_MIN}, {X_MAX}], Y_log={Y_LOG}")
    ...
    _KEEPER.extend([g_syst, g_ratio_band])
    ...
    h_dummy.GetYaxis().SetTitle("Events / bin"); h_dummy.GetYaxis().SetTitleSize(0.055); h_dummy.GetYaxis().SetTitleOffset(1.1)
    ...
    leg = ROOT.TLegend(0.55, 0.60, 0.92, 0.88); leg.SetBorderSize(0); leg.SetFillStyle(0); leg.SetTextSize(0.035)
    ...
    leg.AddEntry(g_syst, "Stat + Syst Unc.", "f"); leg.Draw()
    ...
    h_frame.GetYaxis().SetTitle("Data / MC"); h_frame.SetMinimum(0.5); h_frame.SetMaximum(1.5)
    h_frame.GetXaxis().SetTitle(HIST_NAME.split("_")[-1] + " [GeV]"); h_frame.GetXaxis().SetTitleSize(0.13); h_frame.GetXaxis().SetLabelSize(0.11)
    h_frame.GetYaxis().SetTitleSize(0.12); h_frame.GetYaxis().SetLabelSize(0.11); h_frame.GetYaxis().SetTitleOffset(0.45)
    ...
    if IS_BLIND:
        ...
        g_ratio_band.SetFillColorAlpha(ROOT.kBlack, 0.6); g_ratio_band.SetFillStyle(3013); g_ratio_band.Draw("SAME E2")
