...
void DY_backup::initializeAnalyzer() {
    cout << "[DY_backup::initializeAnalyzer] Starting initialization" << endl;
    
    // Check user flags
    RunSyst = HasFlag("RunSyst");
    
    cout << "[DY_backup::initializeAnalyzer] RunSyst = " << RunSyst << endl;
    ...
    if (DataEra == "2016preVFP" || DataEra == "2016postVFP" || DataEra == "2018") {
        ...
        cerr << "[DY_backup::initializeAnalyzer] DataEra is not set properly: " << DataEra << endl;
        exit(EXIT_FAILURE);
    }
    
    cout << "[DY_backup::initializeAnalyzer] IsoMuTriggerName = " << IsoMuTriggerName << endl;
    cout << "[DY_backup::initializeAnalyzer] TriggerSafePtCut = " << TriggerSafePtCut << endl;
    ...
    cout << "[DY_backup::initializeAnalyzer] Initialization complete" << endl;
}

void DY_backup::executeEvent() {
...
void DY_backup::executeEventFromParameter() {
...
RVec<Muon> DY_backup::SelectMuons(const RVec<Muon>& muons) {
...
RVec<Muon> DY_backup::SelectMuonssublead(const RVec<Muon>& muons) {
    RVec<Muon> selected_muons;
    for (const auto& muon : muons) {
...
RVec<Muon> DY_backup::RemoveOverlap(const RVec<Muon>& muons) {
