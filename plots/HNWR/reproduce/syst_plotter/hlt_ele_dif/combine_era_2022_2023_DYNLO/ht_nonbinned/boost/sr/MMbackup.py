...
def run_plot():
    ...
    signal_hists = {}   # { fname_base: TH1 (Central) }
    ...
    # ── 시스테마틱 계산 ─────────────────────────────────────────────────
    print(">> Calculating Systematic Uncertainties (MC + Signal)...")
    n_bins    = h_total_mc.GetNbinsX()
    sum_sq_up = np.zeros(n_bins + 2)
    sum_sq_dn = np.zeros(n_bins + 2)

    # 전체 배경 MC: Up/Down 분리 저장
    syst_per_bin_up = {syst: np.zeros(n_bins + 2) for syst in SYST_LIST}
    syst_per_bin_dn = {syst: np.zeros(n_bins + 2) for syst in SYST_LIST}
    ...
    syst_per_bin_up_group = {syst: {g: np.zeros(n_bins + 2) for g in BACKGROUND_GROUPS} for syst in SYST_LIST}
    syst_per_bin_dn_group = {syst: {g: np.zeros(n_bins + 2) for g in BACKGROUND_GROUPS} for syst in SYST_LIST}

    # 시그널별 시스테마틱 저장소
    sorted_sig_names      = sorted(signal_hists.keys())
    sig_sum_sq_up         = {sig: np.zeros(n_bins + 2) for sig in sorted_sig_names}
    sig_sum_sq_dn         = {sig: np.zeros(n_bins + 2) for sig in sorted_sig_names}
    sig_syst_per_bin_up   = {syst: {sig: np.zeros(n_bins + 2) for sig in sorted_sig_names} for syst in SYST_LIST}
    sig_syst_per_bin_dn   = {syst: {sig: np.zeros(n_bins + 2) for sig in sorted_sig_names} for syst in SYST_LIST}

    for syst in SYST_LIST:
        h_mc_up, h_mc_dn = None, None
        h_group_up = {g: None for g in BACKGROUND_GROUPS}
        h_group_dn = {g: None for g in BACKGROUND_GROUPS}
        h_sig_up   = {sig: None for sig in sorted_sig_names}
        h_sig_dn   = {sig: None for sig in sorted_sig_names}
        ...
        for fname in all_files:
            ...
            path      = os.path.join(DEFAULT_DATA_PATH, fname)
            is_signal = fname_base.startswith("WR")
            is_data   = any(df in fname for df in DATA_FILES)
            ...
            if is_signal:
                if SCRIPT_CHANNEL and not fname_base.endswith(SCRIPT_CHANNEL):
                    continue
                if fname_base not in sorted_sig_names:
                    continue

                h_sig_nom = signal_hists[fname_base]
                ...
                # 해당 시스테마틱 없으면 nominal로 대체 (변화 없음)
                if u is None:
                    u = h_sig_nom.Clone(f"h_signom_as_up_{fname_base}_{syst}")
                    u.SetDirectory(0)
                if d is None:
                    d = h_sig_nom.Clone(f"h_signom_as_dn_{fname_base}_{syst}")
                    d.SetDirectory(0)

                h_sig_up[fname_base] = u
                h_sig_dn[fname_base] = d
                continue
            # ────────────────────────────────────────────────────────────
            ...
            h_nom = get_hist_from_file(path, "Central", HIST_NAME, X_MAX, CUSTOM_BINS, REBIN_FACTOR)
            if h_nom is None:
                continue
            ...
            if u is None:
                u = h_nom.Clone(f"h_nom_as_up_{fname_base}_{syst}"); u.SetDirectory(0)
            if d is None:
                d = h_nom.Clone(f"h_nom_as_dn_{fname_base}_{syst}"); d.SetDirectory(0)

            # 전체 MC 누적
            if h_mc_up is None:
                h_mc_up = u.Clone("h_up"); h_mc_up.SetDirectory(0)
            else:
                h_mc_up.Add(u)
            if h_mc_dn is None:
                h_mc_dn = d.Clone("h_dn"); h_mc_dn.SetDirectory(0)
            else:
                h_mc_dn.Add(d)
        ...
        for i in range(1, n_bins + 1):
            ...
            if h_mc_up and h_mc_dn:
                ...
                up_contrib = max(0.0, diff_up)
                dn_contrib = max(0.0, -diff_dn)

                syst_per_bin_up[syst][i] = (up_contrib / nom * 100) if nom > 0 else 0.0
                syst_per_bin_dn[syst][i] = (dn_contrib / nom * 100) if nom > 0 else 0.0

                sum_sq_up[i] += up_contrib**2
                sum_sq_dn[i] += dn_contrib**2
            ...
            for g in BACKGROUND_GROUPS:
                if h_group_up[g] and h_group_dn[g] and group_hists[g]:
                    ...
                    g_up_contrib = max(0.0, g_dif_up)
                    g_dn_contrib = max(0.0, -g_dif_dn)

                    syst_per_bin_up_group[syst][g][i] = (g_up_contrib / g_nom * 100) if g_nom > 0 else 0.0
                    syst_per_bin_dn_group[syst][g][i] = (g_dn_contrib / g_nom * 100) if g_nom > 0 else 0.0

                    sum_sq_up_group[g][i] += g_up_contrib**2
                    sum_sq_dn_group[g][i] += g_dn_contrib**2

            # 시그널별
            for sig in sorted_sig_names:
                if h_sig_up[sig] and h_sig_dn[sig]:
                    s_nom        = signal_hists[sig].GetBinContent(i)
                    s_dif_up     = h_sig_up[sig].GetBinContent(i) - s_nom
                    s_dif_dn     = h_sig_dn[sig].GetBinContent(i) - s_nom

                    s_up_contrib = max(0.0, s_dif_up)
                    s_dn_contrib = max(0.0, -s_dif_dn)

                    sig_syst_per_bin_up[syst][sig][i] = (s_up_contrib / s_nom * 100) if s_nom > 0 else 0.0
                    sig_syst_per_bin_dn[syst][sig][i] = (s_dn_contrib / s_nom * 100) if s_nom > 0 else 0.0

                    sig_sum_sq_up[sig][i] += s_up_contrib**2
                    sig_sum_sq_dn[sig][i] += s_dn_contrib**2
        # ─────────────────────────────────────────────────────────────────
    ...
    for i in range(1, n_bins + 1):
        ...
        if y > 0:
            ...
            # ── row_data 구성 ─────────────────────────────────────────
            row_data = [i,
                        h_total_mc.GetBinLowEdge(i), h_total_mc.GetBinLowEdge(i+1),
                        y, data_val,
            ...
            # 전체 배경 MC 시스테마틱 Up/Down
            for syst in SYST_LIST:
                row_data.append(syst_per_bin_up[syst][i])
                row_data.append(syst_per_bin_dn[syst][i])
            ...
            for g in BACKGROUND_GROUPS:
                ...
                for syst in SYST_LIST:
                    row_data.append(syst_per_bin_up_group[syst][g][i])
                    row_data.append(syst_per_bin_dn_group[syst][g][i])

            # 시그널별 시스테마틱
            for sig in sorted_sig_names:
                s_nom     = signal_hists[sig].GetBinContent(i)
                s_stat    = signal_hists[sig].GetBinError(i)
                s_syst_up = math.sqrt(sig_sum_sq_up[sig][i])
                s_syst_dn = math.sqrt(sig_sum_sq_dn[sig][i])
                s_err_up  = math.sqrt(s_stat**2 + sig_sum_sq_up[sig][i])
                s_err_dn  = math.sqrt(s_stat**2 + sig_sum_sq_dn[sig][i])
                row_data.append(s_nom)
                row_data.append((s_stat    / s_nom * 100) if s_nom > 0 else 0.0)
                row_data.append((s_syst_up / s_nom * 100) if s_nom > 0 else 0.0)
                row_data.append((s_syst_dn / s_nom * 100) if s_nom > 0 else 0.0)
                row_data.append((s_err_up  / s_nom * 100) if s_nom > 0 else 0.0)
                row_data.append((s_err_dn  / s_nom * 100) if s_nom > 0 else 0.0)
                for syst in SYST_LIST:
                    row_data.append(sig_syst_per_bin_up[syst][sig][i])
                    row_data.append(sig_syst_per_bin_dn[syst][sig][i])
            # ─────────────────────────────────────────────────────────
            csv_rows.append(row_data)

    print("="*160 + "\n")

    # Per-systematic 요약 출력 (배경 MC)
    print(">> Per-Systematic Contribution Summary — Background MC")
    print("-"*70)
    valid_bins = [i for i in range(1, n_bins + 1) if h_total_mc.GetBinContent(i) > 0]
    syst_avg = []
    for syst in SYST_LIST:
        avg_up = np.mean([syst_per_bin_up[syst][i] for i in valid_bins]) if valid_bins else 0
        avg_dn = np.mean([syst_per_bin_dn[syst][i] for i in valid_bins]) if valid_bins else 0
        syst_avg.append((syst, avg_up, avg_dn))
        print(f"  {syst:<15}: Up {avg_up:7.2f}%  /  Dn {avg_dn:7.2f}%")
    syst_avg.sort(key=lambda x: max(x[1], x[2]), reverse=True)
    print("-"*70)
    print(">> Ranked by max(Up, Dn):")
    for rank, (s, avg_up, avg_dn) in enumerate(syst_avg, 1):
        print(f"  {rank:>2}. {s:<15}: Up {avg_up:7.2f}%  /  Dn {avg_dn:7.2f}%")
    print("="*70 + "\n")

    # Per-systematic 요약 출력 (시그널)
    for sig in sorted_sig_names:
        valid_sig_bins = [i for i in range(1, n_bins + 1) if signal_hists[sig].GetBinContent(i) > 0]
        print(f">> Per-Systematic Contribution Summary — Signal: {sig}")
        print("-"*70)
        sig_avg = []
        for syst in SYST_LIST:
            avg_up = np.mean([sig_syst_per_bin_up[syst][sig][i] for i in valid_sig_bins]) if valid_sig_bins else 0
            avg_dn = np.mean([sig_syst_per_bin_dn[syst][sig][i] for i in valid_sig_bins]) if valid_sig_bins else 0
            sig_avg.append((syst, avg_up, avg_dn))
            print(f"  {syst:<15}: Up {avg_up:7.2f}%  /  Dn {avg_dn:7.2f}%")
        sig_avg.sort(key=lambda x: max(x[1], x[2]), reverse=True)
        print("-"*70)
        print(">> Ranked by max(Up, Dn):")
        for rank, (s, avg_up, avg_dn) in enumerate(sig_avg, 1):
            print(f"  {rank:>2}. {s:<15}: Up {avg_up:7.2f}%  /  Dn {avg_dn:7.2f}%")
        print("="*70 + "\n")
    ...
    with open(tsv_file, "w") as tsvf:
        header = ("Bin\tBinLow\tBinHigh\tMC_Total\tData\t"
                  "DYJets\tTT\tNonprompt\tOthers\t"
                  "Stat_pct\tSystUp_pct\tSystDn_pct\tTotalUp_pct\tTotalDn_pct")
        # 전체 배경 MC 시스테마틱 Up/Down
        for syst in SYST_LIST:
            header += f"\t{syst}_Up_pct\t{syst}_Dn_pct"
        # 그룹별
        for g in BACKGROUND_GROUPS:
            header += f"\t{g}_Stat_pct\t{g}_SystUp_pct\t{g}_SystDn_pct\t{g}_TotalUp_pct\t{g}_TotalDn_pct"
            for syst in SYST_LIST:
                header += f"\t{g}_{syst}_Up_pct\t{g}_{syst}_Dn_pct"
        # 시그널별 (yield + stat/syst + 개별 시스테마틱 Up/Down)
        for sig in sorted_sig_names:
            header += f"\t{sig}"
            header += f"\t{sig}_Stat_pct\t{sig}_SystUp_pct\t{sig}_SystDn_pct\t{sig}_TotalUp_pct\t{sig}_TotalDn_pct"
            for syst in SYST_LIST:
                header += f"\t{sig}_{syst}_Up_pct\t{sig}_{syst}_Dn_pct"
        tsvf.write(header + "\n")
        for row in csv_rows:
    ...
    latex.DrawLatex(0.18, 0.82, "mumu")
