...
DEFAULT_DATA_PATH = "/gv0/Users/achihwan/SKNanoOutput/Reproduce20_002_copy/2022/"
...
DATA_FILES = ["EGamma"]
EXCLUDE_SAMPLES = ["Skim", "Muon", "SingleMuon", "EGamma"]
...
def run_plot():
    ...
    bg_int_row = list(int_row)  # 시그널 추가 전 배경 전용 행 저장
    ...
    for sig_name in sorted(signal_hists.keys()):
        ...
        if s_nom_int > 0:
            ...
            _sig_systs = [syst_per_int_sig[syst][sig_name] for syst in SYST_LIST]
            int_row += _sig_vals
            for v in _sig_systs:
                int_row.append(v)
            sig_int_rows[sig_name] = [s_nom_int] + _sig_vals + _sig_systs
        else:
            int_row += [0.0] * (5 + len(SYST_LIST))
            sig_int_rows[sig_name] = [s_nom_int] + [0.0] * (5 + len(SYST_LIST))
    ...
    # --- 시그널 샘플별 개별 TSV (signal/ 폴더) ---
    _script_dir = os.path.dirname(os.path.abspath(__file__))
    _subdir  = os.path.basename(_script_dir)                                          # e.g. "sig"
    _parent  = os.path.basename(os.path.dirname(_script_dir))                         # e.g. "Resolve"
    _year    = os.path.basename(os.path.dirname(os.path.dirname(_script_dir)))        # e.g. "2022"
    _channel = SCRIPT_CHANNEL or os.path.splitext(os.path.basename(__file__))[0]     # "EE" or "MM"
    _sig_prefix = f"{_parent}{_subdir.capitalize()}{_channel}_{_year}"

    out_dir = os.path.dirname(PLOT_OUT_NAME) or "."
    signal_dir = os.path.join(out_dir, "signal")
    os.makedirs(signal_dir, exist_ok=True)

    # 배경 전용 헤더 (시그널 열 제외) — ResolveSR*_integrated.tsv 와 동일 형식
    bg_header = "Bin\tBinLow\tBinHigh\tMC_Total\tData\tDYJets\tTT\tNonprompt\tOthers\tStat_pct\tSystUp_pct\tSystDn_pct\tTotalUp_pct\tTotalDn_pct"
    for syst in SYST_LIST:
        bg_header += f"\t{syst}_pct"
    for g in BACKGROUND_GROUPS:
        bg_header += f"\t{g}_Stat_pct\t{g}_SystUp_pct\t{g}_SystDn_pct\t{g}_TotalUp_pct\t{g}_TotalDn_pct"
        for syst in SYST_LIST:
            bg_header += f"\t{g}_{syst}_pct"

    for sig_name, sig_row in sig_int_rows.items():
        sig_col_header = f"\t{sig_name}\t{sig_name}_Stat_pct\t{sig_name}_SystUp_pct\t{sig_name}_SystDn_pct\t{sig_name}_TotalUp_pct\t{sig_name}_TotalDn_pct"
        for syst in SYST_LIST:
            sig_col_header += f"\t{sig_name}_{syst}_pct"
        full_header = bg_header + sig_col_header
        full_row    = bg_int_row + sig_row
        sig_tsv = os.path.join(signal_dir, f"{_sig_prefix}_{sig_name}_integrated.tsv")
        with open(sig_tsv, "w") as sf:
            sf.write(full_header + "\n")
            sf.write("\t".join(f"{v:.4f}" if isinstance(v, float) else str(v) for v in full_row) + "\n")
        print(f">> Signal TSV saved as {sig_tsv}")
