...
void ExampleRun::initializeAnalyzer() {
  ...
  // if you use "--userflags RunSyst" with SKFlat.py, HasFlag("RunSyst") will
  // return true
  ...
  // At this point, sample informations (e.g. IsDATA, DataStream, MCSample, or
  // DataEra) are all set You can define sample-dependent or era-dependent
  // variables here example: era-dependent variables "TString IsoMuTriggerName;"
  // and "float TriggerSafePtCut;" are defined in Analyzers/include/ExampleRun.h
  // IsoMuTriggerName is an era-dependent variable, which is used throhoughout
  // the events(let's make it global variable)
  ...
  cout << "[ExampleRun::initializeAnalyzer] TriggerSafePtCut = "
  ...
  if (IsDATA) {
    ...
    systHelper = std::make_unique<SystematicHelper>(
        SKNANO_HOME + "/docs/ExampleSystematic.yaml", MCSample, DataEra);
  }

  //==== Example 2
  //==== Using new PDF
  //==== It consumes so much time, so only being activated with --userflags
  //RunNewPDF
  ...
  if (RunNewPDF && !IsDATA) {
    ...
    pdfReweight->SetNewPDFAlphaS(LHAPDFHandler_New.PDFAlphaS_Down,
                                 LHAPDFHandler_New.PDFAlphaS_Up);
  }
  ...
  RunXSecSyst = false;
  cout << "[ExampleRun::initializeAnalyzer] RunXSecSyst = " << RunXSecSyst
       << endl;
}
...
void ExampleRun::executeEventFromParameter() {
  ...
  if (this_syst == "Central") {
    ...
    // I turned on OneSided option in the yaml file, then it variates in "Up"
    // variation and stays same in "Down" variation
    this_muon_id = MuonIDs[1];
    this_muon_id_sf_key = MuonIDSFKeys[1];
  } else {
    // cerr << "[ExampleRun::executeEventFromParameter] check syst.yaml to set
    // the systematic source for " << this_syst << endl; exit(EXIT_FAILURE);
    return;
  }
