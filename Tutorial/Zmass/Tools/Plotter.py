...
class KinematicCanvas():
    ...
    def drawBackgrounds(self, hists, colors):
        ...
        if self.logy:
            self.stack.GetHistogram().GetYaxis().SetRangeUser(1, maximum*500.)
            for hist in self.signals.values():
                hist.GetYaxis().SetRangeUser(1, maximum*500.)
        else:
...
class ComparisonCanvas():
    ...
    def drawBackgrounds(self, hists, colors):
        ...
        for hist in self.backgrounds.values():
            if self.logy: hist.GetYaxis().SetRangeUser(1, maximum*100.)
    ...
    def drawData(self, data):
        ...
        maximum = self.stack.GetHistogram().GetMaximum() 
        if self.logy: self.data.GetYaxis().SetRangeUser(1, maximum*100.)
