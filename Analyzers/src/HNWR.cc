...
float HNWR::GetElectronTriggerSF_TnP(double eta, double pt, MyCorrection::variation var) const {
    ...
    else return 1.0; // no egamma-tnp measurement for this era (e.g. 2017, 2024)
...
void HNWR::executeEventFromParameter() {
    const TString this_syst = systHelper->getCurrentSysName();
    
    Event ev = GetEvent();
    Particle METv = ev.GetMETVector(Event::MET_Type::PUPPI, MyCorrection::variation::nom, Event::MET_Syst::CENTRAL);
    ...
    // Muon::TightCharge() was removed from DataFormats/Muon.h during the
    // lazy-loading/view-based refactor (commits 8c44c73, 3f17018) and never
    // reintroduced, so every `->TightCharge()` call on a Muon* below is
    // commented out and muon{1,2}_tight_charge keep their default value (1),
    // making all `== 2` "tight charge" checks on muons always false for now.
    // To restore: re-add j_tightCharge + TightCharge()/SetTightCharge() to
    // Muon.h/Muon.cc (see old implementation in commit 2fe7069) and make sure
    // the NanoAOD Muon_tightCharge branch is read into it via the SoA/loader
    // path, then uncomment the assignments marked "TightCharge() removed".
    ...
    FillHist(this_syst + "/Fatjet_SDM_beforecut" , fatjets.size() > 0 ? fatjets[0].GetSDM() : 0. , weight, 100, 0., 500.);
    FillHist(this_syst + "/Fatjet_LSF3_beforecut" , fatjets.size() > 0 ? fatjets[0].LSF3() : 0. , weight, 100, 0., 1.);
    

    // FatJet has no PassID() of its own; pt/eta/ID selection is done via
    // AnalyzerCore::SelectFatJets (uses myCorr->PassFatJetID under the hood),
    // SDM and LSF3 cuts are applied on top of that here.
    RVec<FatJet> fatjets_passID = SelectFatJets(fatjets, fatjet_set.FatJet_ID, fatjet_set.FatJet_MinPt, fatjet_set.FatJet_MaxEta);
    for (unsigned int i=0 ; i< fatjets_passID.size(); i ++) {

        FatJet & fj = fatjets_passID.at(i);
        if (fj.GetSDM() > fatjet_set.FatJet_SDM) {
    ...
    //jet veto 
    bool is_jet_veto = AnalyzerCore::PassJetVetoMap(jets, mu_set.AllMuons, "jetvetomap");
    ...
    if ( (n_Tight_leptons == 2 ) && (Tight_leps[0]->Pt() > 60.0)  && (Tight_leps[1]->Pt() > 53.0)) {
        ...
        if ( (Tight_electrons.size() == 2) && ( Tight_muons.size() == 0 )) {
            ...
            if (DataEra == "2024") {
                return el_set.GetHEEP2024SF(Tight_electrons[0]->Pt(), Tight_electrons[0]->Eta(), var)
                     * el_set.GetHEEP2024SF(Tight_electrons[1]->Pt(), Tight_electrons[1]->Eta(), var);
            }
        ...
        if (this_trigger_pass) {
            ...
            FillHist(this_syst + "/Jetnumber_before_resolved_selection1", selected_jets.size(), weight, 20,-10,10.);
                ...
                if(!IsDATA){
                    if(tmp_isEE){
                        ...
                        weight_function_map["E_Trig_Weight"] = [&](MyCorrection::variation var, TString source)  {
                            if (DataEra == "2024") {
                                return el_set.GetTrigger2024SF(Tight_electrons[0]->Pt(), Tight_electrons[0]->Eta(), var);
                            }
                ...
                if ( DiLepMassLT150 && WRCand.M() > 800.0 ) {
                    if (tmp_isEE) {
                        ...
                        // TightCharge() removed (see note above muon1_tight_charge declaration)
                        ...
                        float tightmuon1_org_pt = Tight_muons[0]->MiniAODPt();
            ...
            // DY CR2 ( 100 < mll < 150)
                ...
                if ( DiLepMassGT400 && WRCand.M() > 800.0 ){
                    ...
                    // muon1_tight_charge = Tight_muons[0]->TightCharge() ; // TightCharge() removed, see note above
        ...
        if ((n_Tight_leptons >0 ) && (Tight_leps[0]->Pt() > 60.0)) {
                    ...
                    for (unsigned int i=0 ; i< fatjets.size(); i++) {
                        ...
                        // if loose lepton is inside of fatjet
                            if (this_fatjet.DeltaR( *LowMllLooseLepton)<0.8) {
                                ...
                                FillHist(this_syst +"/numofhnfatjet_DY", HNFatJet.GetSDM() , weight, 10000, 0., 10000.);
                            }
                            Particle WRCand;
                            ...
                            if ( WRCand.M() > 800.0 )  {
                                ...
                                // DY CR
                                FillHist(this_syst + "/DY_CR_Fatjet_SDMass", HNFatJet.GetSDM() , weight, 10000, 0., 10000.);
                                if (!IsDATA){
                                    //SF
                                    if(is_tmp_lead_el){
                                ...
                                if (is_tmp_lead_el){ // ee 
                                        ...
                                        Boost_DYCREEfatjetSDM  = HNFatJet.GetSDM();
                                        ...
                                        if (looselepton_infatjet){
                                            FillHist(this_syst + "/Boosted_DY_CR_EE_looselepton_infatjet_Fatjet_SDMass", HNFatJet.GetSDM() , weight, 10000, 0., 10000.);
                                        }
                                        else{
                                            FillHist(this_syst + "/Boosted_DY_CR_EE_looselepton_outsidefatjet_Fatjet_SDMass", HNFatJet.GetSDM() , weight, 10000, 0., 10000.);
                                        }
                                    ...
                                    if (is_tmp_lead_mu){// mumu
                                        ...
                                        Boost_DYCRMMfatjetSDM  = HNFatJet.GetSDM();
                                        ...
                                        if (looselepton_infatjet){
                                            FillHist(this_syst + "/Boosted_DY_CR_MM_looselepton_infatjet_Fatjet_SDMass", HNFatJet.GetSDM() , weight, 10000, 0., 10000.);
                                        }
                                        else{
                                            FillHist(this_syst + "/Boosted_DY_CR_MM_looselepton_outsidefatjet_Fatjet_SDMass", HNFatJet.GetSDM() , weight, 10000, 0., 10000.);
                                        }
                                    }
            ...
            // dont have low mll ( > 150 )
                    ...
                    if (hasawaymergedfatjet) {
                        ...
                        FillHist(this_syst +"/numofhnfatjet_FLV", HNFatJet.GetSDM() , weight, 10000, 0., 10000.); 
                                        ...
                                        if (WRMassGT800) {
                                            if(!IsDATA){
                                                ...
                                                if (is_tmp_lead_el) {
                                                    ...
                                                    Boost_SREEfatjetSDM  = HNFatJet.GetSDM();
                                                ...
                                                if (is_tmp_lead_mu) {
                                                    ...
                                                    Boost_SRMMfatjetSDM  = HNFatJet.GetSDM();
                                ...
                                if ((*LeadLep + *OFLooseLepton).M() > 200.0) {
                                    ...
                                    if(is_tmp_lead_mu){
                                        ...
                                        if (is_tmp_lead_el) {
                                            ...
                                            // muon1_tight_charge = ((Muon*)OFLooseLepton)->TightCharge() ; // TightCharge() removed, see note above
                                            ...
                                            Boost_FlavEMJSDM = HNFatJet.GetSDM();
                                            ...
                                            */
                                            ...
                                            bool tight_charge = false; // TightCharge() removed, see note above (was: Tight_muons[0]->TightCharge() == 2)
                                            if (LeadLep->Charge() * OFLooseLepton->Charge() > 0) {
                                            ...
                                            Boost_FlavMEJSDM = HNFatJet.GetSDM();
...
void HNWR::FillSignalCutflow(const TString &this_syst, bool isResolved, double binN, float weight) {
...
bool HNWR::Electrons::isPassCustomTightID(const Electron& el, const HNWR::Electrons& eset) const {
...
bool HNWR::Electrons::isPassCustomLooseID(const Electron& el) const {
...
bool HNWR::Electrons::isPassLooseNoIso(const Electron& el) const {
...
namespace {
    // 2024 EGamma Tag&Probe scale factors.
    // Source: Alpana_WR_LRSM_update_22June2026.pdf (UMN subgroup meeting, 22 June 2026),
    //         transcribed into tnpsf/WR_efficiencies_SF_errors.xlsx (slides 2 & 6).
    // Eta columns are SIGNED eta (matches how the T&P fit was binned), crack |eta| in [1.44,1.57] excluded:
    //   col0: [-2.5,-1.57]  col1: [-1.44,0]  col2: [0,1.44]  col3: [1.57,2.5]
    int GetEEEtaColumn2024(float eta) {
        if (eta >= -2.5f && eta <= -1.57f) return 0;
        if (eta >= -1.44f && eta <= 0.f)   return 1;
        if (eta >  0.f    && eta <= 1.44f) return 2;
        if (eta >= 1.57f  && eta <= 2.5f)  return 3;
        return -1; // ECAL crack or out of acceptance: no measurement, caller returns SF=1
    }

    template <int NPt>
    int GetPtBin2024(float pt, const float (&ptLowEdge)[NPt]) {
        for (int i = 0; i < NPt; i++) {
            if (pt >= ptLowEdge[i]) return i;
        }
        return NPt - 1; // clamp underflow to the lowest measured bin
    }

    // HEEP ID SF: pT bins ordered high -> low (low edge of each bin); >300 GeV bin uses 300 as its edge.
    constexpr int kHEEPNPt = 11;
    constexpr float kHEEPPtLowEdge[kHEEPNPt] = {300, 200, 100, 90, 80, 70, 60, 50, 45, 40, 35};
    constexpr float kHEEPSF[kHEEPNPt][4] = {
        {0.967f, 0.997f, 0.997f, 1.017f},
        {0.954f, 0.993f, 0.987f, 0.992f},
        {0.948f, 0.980f, 0.976f, 0.991f},
        {0.952f, 0.980f, 0.981f, 0.992f},
        {0.954f, 0.983f, 0.982f, 0.997f},
        {0.954f, 0.982f, 0.977f, 0.999f},
        {0.946f, 0.974f, 0.960f, 0.991f},
        {0.938f, 0.966f, 0.963f, 0.981f},
        {0.929f, 0.954f, 0.956f, 0.976f},
        {0.942f, 0.962f, 0.953f, 0.982f},
        {0.933f, 0.957f, 0.952f, 0.979f},
    };
    // Relative SF error [%] as labeled on the T&P plots.
    constexpr float kHEEPSFRelErrPct[kHEEPNPt][4] = {
        {1.958f, 0.747f, 0.258f, 1.684f},
        {0.884f, 0.408f, 0.369f, 0.743f},
        {0.238f, 0.045f, 0.106f, 0.210f},
        {0.365f, 0.178f, 0.165f, 0.320f},
        {0.290f, 0.139f, 0.130f, 0.256f},
        {0.230f, 0.093f, 0.088f, 0.200f},
        {0.131f, 0.061f, 0.056f, 0.118f},
        {0.063f, 0.030f, 0.011f, 0.058f},
        {0.056f, 0.022f, 0.021f, 0.040f},
        {0.033f, 0.024f, 0.019f, 0.036f},
        {0.041f, 0.025f, 0.024f, 0.038f},
    };

    // Electron trigger SF: pT bins ordered high -> low (low edge of each bin).
    constexpr int kTrigNPt = 11;
    constexpr float kTrigPtLowEdge[kTrigNPt] = {300, 200, 140, 120, 100, 90, 70, 60, 50, 40, 30};
    constexpr float kTrigSF[kTrigNPt][4] = {
        {0.993f, 0.995f, 0.998f, 0.993f},
        {0.997f, 0.995f, 0.996f, 0.994f},
        {0.990f, 0.992f, 0.993f, 0.991f},
        {0.991f, 0.994f, 0.996f, 0.989f},
        {0.993f, 0.999f, 0.997f, 0.990f},
        {0.996f, 1.002f, 1.002f, 0.997f},
        {0.998f, 1.007f, 1.006f, 0.998f},
        {0.999f, 1.010f, 1.008f, 0.999f},
        {1.002f, 0.997f, 0.996f, 1.004f},
        {1.013f, 0.980f, 0.977f, 1.011f},
        {1.008f, 0.928f, 0.974f, 1.007f},
    };
    constexpr float kTrigSFRelErrPct[kTrigNPt][4] = {
        {0.123f, 0.069f, 0.245f, 0.373f},
        {0.092f, 0.100f, 0.097f, 0.151f},
        {0.028f, 0.096f, 0.076f, 0.114f},
        {0.129f, 0.115f, 0.086f, 0.157f},
        {0.110f, 0.088f, 0.064f, 0.133f},
        {0.129f, 0.042f, 0.074f, 0.146f},
        {0.066f, 0.047f, 0.040f, 0.078f},
        {0.051f, 0.037f, 0.034f, 0.052f},
        {0.046f, 0.024f, 0.018f, 0.035f},
        {0.027f, 0.017f, 0.016f, 0.029f},
        {0.046f, 0.027f, 0.025f, 0.046f},
    };

    float ApplyVariation2024(float sf, float relErrPct, MyCorrection::variation var) {
        const float absErr = sf * (relErrPct / 100.f);
        if (var == MyCorrection::variation::up)   return sf + absErr;
        if (var == MyCorrection::variation::down) return sf - absErr;
        return sf;
    }
}

float HNWR::Electrons::GetHEEP2024SF(float pt, float eta, MyCorrection::variation var) const {
    const int col = GetEEEtaColumn2024(eta);
    if (col < 0) return 1.f; // ECAL crack / out of acceptance: no correction available
    const int row = GetPtBin2024(pt, kHEEPPtLowEdge);
    return ApplyVariation2024(kHEEPSF[row][col], kHEEPSFRelErrPct[row][col], var);
}

float HNWR::Electrons::GetTrigger2024SF(float pt, float eta, MyCorrection::variation var) const {
    const int col = GetEEEtaColumn2024(eta);
    if (col < 0) return 1.f; // ECAL crack / out of acceptance: no correction available
    const int row = GetPtBin2024(pt, kTrigPtLowEdge);
    return ApplyVariation2024(kTrigSF[row][col], kTrigSFRelErrPct[row][col], var);
}

RVec<FatJet> HNWR::Clean_Fatjet_with_tight_leptons(const RVec<FatJet> & fatjets, const RVec<Lepton *> & tight_leps) {
...
RVec<Jet> HNWR::Clean_jet_with_loose_leptons(const RVec<Jet> & jets, const RVec<Lepton *> & loose_leps) {
...
RVec<Jet> HNWR::Clean_LSF_FatJet_with_jets(const RVec<FatJet> & fatjets, const RVec<Jet> & jets) {
    RVec<Jet> cleanedjets;
    for (unsigned int i=0 ; i< jets.size(); i ++) {
        Jet jet = jets.at(i);
...
RVec<FatJet> HNWR::Clean_Jets_with_fatjets(const RVec<Jet> & jets, const RVec<FatJet> & fatjets) {
    RVec<FatJet> cleanedfatjets;
    for (unsigned int i=0 ; i< fatjets.size(); i ++) {
        FatJet fatjet = fatjets.at(i);
