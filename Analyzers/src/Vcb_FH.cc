...
RVec<RVec<unsigned int>> Vcb_FH::GetPermutations(const RVec<Jet> &jets)
{
    ...
    {
        if (GetPassedBTaggingWP(jets[i]) >= 2)
        {
...
bool Vcb_FH::PassBaseLineSelection(bool remove_flavtagging_cut, bool loose_cut)
{
    Clear();
...
void Vcb_FH::InferONNX()
{
    ...
    for (size_t i = 0; i < max_jet; i++)
    {
        ...
        if (i < Jets.size())
        {
            ...
            btag.push_back(JetBScore(Jets[i]));
        }
        else
        {
            pt.push_back(0.);
...
bool Vcb_FH::FillONNXRecoInfo(const TString &histPrefix, float weight)
{
    ...
    if (JetBScore(Jets[assignment[4]]) > JetBScore(Jets[assignment[5]]))
    {
        // swap the assignment
    ...
    if (JetBScore(Jets[assignment[3]]) > JetBScore(Jets[assignment[5]]))
    {
        // swap the assignment
