...
def run_plot():
    ...
    parser.add_argument("--show-sb",      action="store_true", default=False,  help="빈별 S/B (Signal/Background) 비율 출력")
    ...
    # 출력 경로에 디렉토리가 포함되어 있으면 생성
    out_dir = os.path.dirname(PLOT_OUT_NAME)
    ...
    # ─── S/B (Signal/Background) 빈별 출력 ────────────────────────────────────
    if args.show_sb and signal_hists:
        sig_names = sorted(signal_hists.keys())

        # 먼저 모든 빈 데이터 수집
        sb_rows = []
        for i in range(1, n_bins + 1):
            bkg_val = h_total_mc.GetBinContent(i)
            if bkg_val <= 0:
                continue
            sig_vals = []
            sb_ratios = []
            for sig_name in sig_names:
                h_sig = signal_hists[sig_name]
                sig_val = h_sig.GetBinContent(i)
                sb_ratio = sig_val / bkg_val if bkg_val > 0 else 0
                sig_vals.append(sig_val)
                sb_ratios.append(sb_ratio)
            row_data = [i, h_total_mc.GetBinLowEdge(i), h_total_mc.GetBinLowEdge(i+1), bkg_val]
            for sv, sr in zip(sig_vals, sb_ratios):
                row_data.extend([sv, sr])
            sb_rows.append((row_data, bkg_val, sig_vals, sb_ratios))

        col_w = 14
        bin_hdrs = [f"Bin{rd[0]}[{rd[1]:.0f},{rd[2]:.0f}]" for (rd, *_) in sb_rows]

        # === Part 1: S/B 비율 (전치: 행=시그널, 열=빈) ===
        print("\n" + "="*120)
        print(">> Bin-by-bin S/B (Signal/Background) Ratio  [행=시그널, 열=빈]")
        print("="*120)
        header_sb = f"{'Signal':<25}"
        for bh in bin_hdrs:
            header_sb += f" | {bh:>{col_w}}"
        print(header_sb)
        print("-" * 120)
        for j, sig_name in enumerate(sig_names):
            short_name = sig_name.replace("WR", "").replace("EE", "").replace("MM", "")
            line = f"{short_name + '_S/B':<25}"
            for (row_data, bkg_val, sig_vals, sb_ratios) in sb_rows:
                line += f" | {sb_ratios[j]:>{col_w}.4f}"
            print(line)
        print("="*120)

        # === Part 2: Background와 Signal 이벤트 수 (전치: 행=카테고리, 열=빈) ===
        print("\n" + "="*120)
        print(">> Bin-by-bin Event Counts (Background & Signal)  [행=카테고리, 열=빈]")
        print("="*120)
        header_cnt = f"{'Category':<25}"
        for bh in bin_hdrs:
            header_cnt += f" | {bh:>{col_w}}"
        print(header_cnt)
        print("-" * 120)
        bkg_line = f"{'BKG':<25}"
        for (row_data, bkg_val, sig_vals, sb_ratios) in sb_rows:
            bkg_line += f" | {bkg_val:>{col_w}.2f}"
        print(bkg_line)
        for j, sig_name in enumerate(sig_names):
            short_name = sig_name.replace("WR", "").replace("EE", "").replace("MM", "")
            line = f"{short_name + '_Sig':<25}"
            for (row_data, bkg_val, sig_vals, sb_ratios) in sb_rows:
                line += f" | {sig_vals[j]:>{col_w}.2f}"
            print(line)
        print("="*120)

        # S/B TSV 저장 (전치: 행=카테고리, 열=빈)
        sb_tsv_file = f"{PLOT_OUT_NAME}_sb.tsv"
        with open(sb_tsv_file, "w") as tsvf:
            tsv_header = "Category"
            for (row_data, bkg_val, sig_vals, sb_ratios) in sb_rows:
                tsv_header += f"\tBin{row_data[0]}[{row_data[1]:.0f},{row_data[2]:.0f}]"
            tsvf.write(tsv_header + "\n")
            bkg_tsv = "BKG"
            for (row_data, bkg_val, sig_vals, sb_ratios) in sb_rows:
                bkg_tsv += f"\t{bkg_val:.4f}"
            tsvf.write(bkg_tsv + "\n")
            for j, sig_name in enumerate(sig_names):
                short_name = sig_name.replace("WR", "").replace("EE", "").replace("MM", "")
                sig_tsv = f"{short_name}_Signal"
                for (row_data, bkg_val, sig_vals, sb_ratios) in sb_rows:
                    sig_tsv += f"\t{sig_vals[j]:.4f}"
                tsvf.write(sig_tsv + "\n")
                sb_tsv = f"{short_name}_S/B"
                for (row_data, bkg_val, sig_vals, sb_ratios) in sb_rows:
                    sb_tsv += f"\t{sb_ratios[j]:.4f}"
                tsvf.write(sb_tsv + "\n")
        print(f">> S/B TSV saved as {sb_tsv_file}\n")
