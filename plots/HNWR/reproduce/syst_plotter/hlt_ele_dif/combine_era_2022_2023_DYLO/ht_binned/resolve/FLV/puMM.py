...
DATA_FILES = [ "Muon", "SingleMuon" ]  # Data 파일명에 공통적으로 포함된 문자열
...
def run_plot():
    ...
    # punum이 포함된 경우 data용 hist name은 pv으로 대체
    # MC는 HIST_NAME 그대로, Data는 DATA_HIST_NAME 사용
    if "punum" in HIST_NAME.lower():
        DATA_HIST_NAME = HIST_NAME.replace("punum", "pv").replace("PUNum", "pv").replace("Punum", "pv")
