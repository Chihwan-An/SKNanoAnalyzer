...
def classify_variable(var: str) -> str:
    ...
    # dilepton: mll / ll_pt / Dilepton_pt / OS|SS_ variants
    if var in ('mll', 'll_pt', 'Dilepton_pt'):
        return 'dilepton'
    ...
    if re.match(r'WR_|mlljj|l1j1j2_|l2j1j2_', var):
        return 'jet_lepton'
    if var in ('WRMass', 'pt', 'eta', 'phi', 'mass'):
        return 'jet_lepton'
...
def organize(dry_run: bool = False):
    ...
    for rel_dir, region_key in SUBDIRS.items():
        ...
        for fname in sorted(os.listdir(src_dir)):
            if not (fname.endswith('.tsv') or fname.endswith('.pdf')):
                continue
