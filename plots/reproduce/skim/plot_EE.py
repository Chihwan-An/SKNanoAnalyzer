...
data_directory = "/gv0/Users/achihwan/SKNanoOutput/Reproduce20_002/SKIM/"
...
def normalize_by_bin_size(hist):
    ...
    # 이산적 변수이거나 빈 크기가 1인 경우 정규화하지 않음
    ...
    normalized_hist.SetDirectory(0)  # 메모리에서 관리되도록 설정
    
    print(f"Normalizing histogram: {hist.GetName()} (bin width: {first_bin_width})")
    
    # 정규화 전 첫 번째 빈의 값 확인
    original_first_bin = hist.GetBinContent(1)
    print(f"Original first bin content: {original_first_bin}")
    
    # 각 빈에 대해 정규화 수행
    for i in range(1, hist.GetNbinsX() + 1):  # 1부터 nbins까지 (0과 nbins+1은 오버플로우/언더플로우)
        # 현재 빈의 내용과 오차 가져오기
    ...
    # 정규화 후 첫 번째 빈의 값 확인
    normalized_first_bin = normalized_hist.GetBinContent(1)
    print(f"Normalized first bin content: {normalized_first_bin}")
    print(f"Normalization factor: {normalized_first_bin/original_first_bin if original_first_bin > 0 else 'N/A'}")
...
def load_scale_factors(sf_file_path):
    ...
    Args:
        sf_file_path: 스케일 팩터 CSV 파일 경로
        
    Returns:
        dict: {hist_name: [(bin_min, bin_max, scale_factor), ...]} 형태의 딕셔너리
    """
    ...
    try:
        ...
        for hist_name, factors in scale_factors.items():
            print(f"  {hist_name}: {len(factors)} bin ranges")
...
def apply_scale_factors(hist, hist_name, scale_factors):
    ...
    Args:
        hist: ROOT 히스토그램 객체
        hist_name: 히스토그램 이름
        scale_factors: load_scale_factors에서 반환된 스케일 팩터 딕셔너리
        
    Returns:
        스케일 팩터가 적용된 히스토그램의 복사본
    """
    ...
    for i in range(1, hist.GetNbinsX() + 1):
        ...
        if scale_factor != 1.0:
            ...
            print(f"  Bin {i} ({bin_low:.1f}-{bin_high:.1f}): {original_content:.3f} -> {scaled_content:.3f} (SF={scale_factor:.3f})")
...
class SignalBackgroundCanvas():
    def __init__(self, combined_data_hist, signal_hists, background_hists, config, draw_tb_lines=True, target_tb_sample=None, normalize_by_bin_size=False):
        super().__init__()
        ...
        self.normalize_by_bin_size = normalize_by_bin_size  # 빈 사이즈로 정규화할지 여부
        ...
        if self.data_hist:
            # 데이터 히스토그램은 이미 combine_Muon_Fata에서 정규화됨
    ...
    def _separate_tb_samples(self):
        ...
        if self.signal_hists:
            for name, hist in self.signal_hists.items():
                if name.startswith("Skim_20002_TB"):
    ...
    def _style_tb_histograms(self):
        ...
        for name, hist in self.tb_hists.items():
            # 정규화는 이미 plot_signal_background_comparison에서 적용됨
    ...
    def _build_background_stack(self):
        ...
        for name, hist in all_hists.items():
            ...
            if name.startswith("Skim_20002_TB") and self.draw_tb_lines:
                # Skip ALL TB samples if TB lines are enabled - they will be drawn as lines only
                continue
            elif name.startswith("Skim_20002_TTLJ"):
                groups["TT+TW"].append((name, hist))
            elif name.startswith("Skim_20002_TTLL"):
                groups["TT+TW"].append((name, hist))
            elif name.startswith("Skim_20002_ST"):
                groups["TT+TW"].append((name, hist))
            elif name.startswith("Skim_20002_DYJets"):
                groups["DYJets"].append((name, hist))
            elif name.startswith("Skim_20002_WJets"):
    ...
    def _setup_canvas(self):
        ...
        # Maximum  groups (TTLJ, TTLL, ST, DYJets, Others) + Data
    ...
    def draw(self):
        ...
        if self.draw_tb_lines:
            for name, hist in self.tb_hists.items():
                ...
                if name.startswith("Skim_20002_TB") and "WR" in name:
        ...
        latex.DrawLatex(0.16, 0.85 ,"ee")
        latex.DrawLatex(0.16, 0.8 ,"Resolved DY CR")
...
def combine_Muon_Fata(data_dir, hist_name, systematic="Central"):
    ...
    for file_path in root_files:
        ...
        if (#filename.startswith("Skim_20002_Muon0") or
            #filename.startswith("Skim_20002_Muon1") or 
            filename.startswith("Skim_20002_EGamma")):
...
def load_background_histograms(data_dir, hist_name, systematic="Central", draw_tb_lines=True):
    ...
    for file_path in root_files:
        ...
        if ((filename.startswith("Skim_20002_Muon") or 
            filename.startswith("Skim_20002_EGamma") or
            filename.startswith("Skim_20002_TB") or
            filename.startswith("Skim_20002_Muon_G.root"))or
            filename.startswith("Skim_20002_Muon_") or
            filename.startswith("Skim_20002_EGamma_") or
            filename.startswith("Skim_20002_MuonEG_") or
            filename.startswith("Skim_20002_SingleMuon") or
            filename.startswith("Skim_20002_DYJets_MG") or
            filename.startswith("Skim_20002_DYG") or
            filename.startswith("Skim_20002_DYJets10to50_MG") ):
            #filename.startswith("Skim_20002_ZZTo") or
            #filename.startswith("Skim_20002_WZTo") or
            #
            #filename.startswith("Skim_20002_TTG") or
            #filename.startswith("Skim_20002_DYJets_MG") or
            #filename.startswith("Skim_20002_DYJets10to50_MG") or
            #filename.endswith("bcToE.root") 
            continue
        
        # Skip TB samples if TB lines are disabled (no-tb-lines option)
        if sample_name.startswith("Skim_20002_TB") and not draw_tb_lines:
...
def get_tb_samples(data_dir):
    ...
    for file_path in root_files:
        filename = os.path.basename(file_path)
        sample_name = filename.replace(".root", "")
        if sample_name.startswith("Skim_20002_TB"):
...
def format_tb_legend_name(tb_sample_name):
    ...
    if tb_sample_name.startswith("Skim_20002_TB") and "WR" in tb_sample_name:
...
def plot_signal_background_comparison(data_dir, hist_name, config, output_name="signal_background_comparison", systematic="Central", draw_tb_lines=True, target_tb_sample=None, normalize_by_bin_size=False, apply_scale_factors_flag=False, sf_file_path=None):
    ...
    # Load scale factors if requested
    scale_factors = {}
    ...
    # Apply normalization after rebinning (if requested)
    if normalize_by_bin_size:
        if combined_data_hist:
            try:
                combined_data_hist = globals()['normalize_by_bin_size'](combined_data_hist)
                print("Data histogram normalized by bin size (after rebinning)")
            except Exception as e:
        ...
        for name, hist in signal_hists.items():
            try:
                ...
                print(f"Signal histogram {name} normalized by bin size (after rebinning)")
            except Exception as e:
        ...
        for name, hist in background_hists.items():
            try:
                ...
                print(f"Background histogram {name} normalized by bin size (after rebinning)")
            except Exception as e:
                print(f"Error normalizing background histogram {name}: {e}")
    
    # Apply scale factors to MC histograms (if requested)
    if apply_scale_factors_flag and scale_factors:
        print(f"Applying scale factors to MC histograms for {hist_name}")
        
        # Apply to signal histograms
        for name, hist in signal_hists.items():
            try:
                scaled_hist = apply_scale_factors(hist, hist_name, scale_factors)
                if scaled_hist != hist:  # Only replace if scaling was applied
                    signal_hists[name] = scaled_hist
                    print(f"Applied scale factors to signal histogram {name}")
            except Exception as e:
                print(f"Error applying scale factors to signal histogram {name}: {e}")
        
        # Apply to background histograms
        for name, hist in background_hists.items():
            try:
                scaled_hist = apply_scale_factors(hist, hist_name, scale_factors)
                if scaled_hist != hist:  # Only replace if scaling was applied
                    background_hists[name] = scaled_hist
                    print(f"Applied scale factors to background histogram {name}")
            except Exception as e:
                print(f"Error applying scale factors to background histogram {name}: {e}")
    else:
        print("Scale factors not applied (disabled or no scale factors loaded)")
    
    # Create and draw the plot
    canvas = None
    try:
        # Store additional info for TB sample loading
        canvas = SignalBackgroundCanvas(combined_data_hist, signal_hists, background_hists, config, draw_tb_lines=draw_tb_lines, target_tb_sample=target_tb_sample, normalize_by_bin_size=normalize_by_bin_size)
